...
def _st_image(image_url=None, title=None, caption=None):
    ...
    title = json.loads(
        requests.get(
            image_url[: image_url.rfind('.') + 1].format(plot_type='line_plot') + 'json'
        ).content
    )['title']
    ...
    if '{plot_type}' in image_url:
        labels = {'Heatmap': 'heat_map', 'Line': 'line_plot'}
        plot_type = labels[
            st.selectbox('Plot type:', ['Heatmap', 'Line'], key=image_url)
        ]
        image_url = image_url.format(plot_type=plot_type)
...
# def _plot_type(key):
#     labels ={'Heatmap':'heat_map', 'Line':'line_plot'}
#     plot_type = labels[st.selectbox('', ['Heatmap', 'Line'], key=key)]
#     return plot_type
...
class PublicHealth:
    ...
    def covid_19_case(plot_type=None):
        ...
        _st_image(
            image_url=f'{PublicHealth.ROOT}/covid_19_case/case_count_by_zip_code_line_plot.pdf'
        )
